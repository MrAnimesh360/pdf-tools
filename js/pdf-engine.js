window.PDFEngine = (() => {
  ...
  const parseOrderedSelection = (value, totalPages) => {
    if (!value?.trim()) return [];
    const out = [];
    value.split(',').map(v => v.trim()).filter(Boolean).forEach(part => {
      if (part.includes('-')) {
        ...
        if (Number.isFinite(start) && Number.isFinite(end)) {
          ...
          for (let i = start; step > 0 ? i <= end : i >= end; i += step) {
            if (i >= 1 && i <= totalPages) out.push(i - 1);
          }
        }
      } else {
        const n = Number(part);
        if (Number.isFinite(n) && n >= 1 && n <= totalPages) out.push(n - 1);
      }
    });
    return out;
  };
  ...
  const addBackgroundToPdf = async (file, options = {}) => {
    const src = await readPdf(file);
    const out = await PDFLib.PDFDocument.create();

    let image;
    if (options.imageBytes) {
      image = options.imageType === 'png'
        ? await out.embedPng(options.imageBytes)
        : await out.embedJpg(options.imageBytes);
    }
    ...
    for (const page of src.getPages()) {
      ...
      if (options.color) outPage.drawRectangle({ x: 0, y: 0, width, height, color: options.color });
      if (image) outPage.drawImage(image, { x: 0, y: 0, width, height, opacity: options.imageOpacity ?? 0.15 });
